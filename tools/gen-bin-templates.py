...
the bytes), and writes:
  ...
  src/gen/bin-templates.inc        64tass include with one record per template
  src/gen/text-template-labels.inc  64tass include with out_* template IDs
  target/basic65t.prg              bank-5 text template image, shipped as +b65tpl
...
OUT_TEXT_LABELS = REPO / "src" / "gen" / "text-template-labels.inc"
OUT_TEXT_PRG = REPO / "target" / "basic65t.prg"
OUT_TXT = REPO / "target" / "bin-templates.txt"
GFX_INC = REPO / "src" / "gfx" / "gfxshared.inc"
TASS = REPO / "64tass.exe"

TEXT_TEMPLATE_BASE = 0xA000
TEXT_TEMPLATE_FAR_BANK = 0x18
TEXT_TEMPLATE_FAR_MB = 0x08
...
def write_text_templates(blobs):
    """Write the text-template disk asset and matching symbolic IDs."""
    addr = TEXT_TEMPLATE_BASE
    image = bytearray((TEXT_TEMPLATE_BASE & 0xff, TEXT_TEMPLATE_BASE >> 8))
    lines = [
        "; generated by tools/gen-bin-templates.py -- do not edit\n",
        "; out_* text-template IDs; bytes live in +b65tpl at bank-5/attic\n",
        f"TEXT_TEMPLATE_BASE = ${TEXT_TEMPLATE_BASE:04x}\n",
        f"TEXT_TEMPLATE_FAR_BANK = ${TEXT_TEMPLATE_FAR_BANK:02x}\n",
        f"TEXT_TEMPLATE_FAR_MB = ${TEXT_TEMPLATE_FAR_MB:02x}\n\n",
    ]

    for name, (text, _) in blobs.items():
        data = text.replace("\n", "\r").encode("ascii")
    ...
    lines.append(f"\nTEXT_TEMPLATE_END = ${addr:04x}\n")
    lines.append(f"TEXT_TEMPLATE_SIZE = ${addr - TEXT_TEMPLATE_BASE:04x}\n")
    OUT_TEXT_LABELS.parent.mkdir(parents=True, exist_ok=True)
    OUT_TEXT_LABELS.write_text("".join(lines), encoding="ascii", newline="\n")
    OUT_TEXT_PRG.parent.mkdir(parents=True, exist_ok=True)
    OUT_TEXT_PRG.write_bytes(image)
...
def main():
    ...
    with OUT_INC.open("w", encoding="ascii", newline="\n") as f:
        ...
        for _nm, _sym in (("RT_END_FIO", "rtendfio"), ("RT_END_MATH", "rtendmath"), ("RT_END_SOUND", "rtendsound"), ("RT_END_OVL", "rtendovl")):
